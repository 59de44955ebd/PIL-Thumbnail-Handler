...
STDAPI DllRegisterServer(void)
{
	...
	hr = RegisterInprocServer(
		szModule, 
		CLSID_ShelExtensionHandler, 
		APPICATION_DESCRIPTION,
		L"Apartment", 
		APPID_ShelExtensionHandler
	);
	if (SUCCEEDED(hr))
	{
		hr = RegisterShellExtThumbnailHandler(FORMAT_EXTENSION1, CLSID_ShelExtensionHandler);
		if (SUCCEEDED(hr))
		{
...
STDAPI DllUnregisterServer(void)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = UnregisterShellExtThumbnailHandler(FORMAT_EXTENSION1);
	}
